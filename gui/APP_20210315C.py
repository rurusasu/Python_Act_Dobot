...
from DobotFunction.Camera import WebCam_OnOff, Snapshot, scale_box
...
class Dobot_APP:
    ...
    def Layout(self):
        ...
        WebCamConnect = [
            ...
            [
                ...
                sg.InputCombo(('なし',
                                        '線形濃度変換',
                                        '非線形濃度変換',  # ガンマ処理
                                        'ヒストグラム平坦化',), default_value="なし", size=(18, 1), key='-Color_Density-', readonly=True),
        ...
        Global_Threshold = [
            ...
            [sg.Text('Threshold', size=(7, 1)), sg.InputText('127', size=(4, 1), justification='right', key='-threshold-')],
        ...
        Adaptive_Threshold = [
            ...
            [sg.InputCombo(('MEAN_C', 'GAUSSIAN_C'), size=(12, 1), key='-AdaptiveThreshold_type-', readonly=True)],
            [sg.Text('Block Size', size=(8, 1)), sg.InputText('11', size=(4, 1), justification='right', key='-AdaptiveThreshold_BlockSize-'), ],
            [sg.Text('Constant', size=(8, 1)), sg.InputText('2', size=(4, 1), justification='right', key='-AdaptiveThreshold_constant-')],
        ]

        # 2つの閾値を用いた二値化
        TwoThreshold = [
            [sg.Radio('TwoThreshold', group_id='threshold', default=True, key='-Twohreshold-'), ],
            [sg.Text('Lower', size=(4, 1)), sg.Slider(range=(0, 127), default_value=10, orientation='horizontal', size=(12, 12), key='-LowerThreshold-'), ],
            [sg.Text('Upper', size=(4, 1)),  sg.Slider(range=(128, 256), default_value=138, orientation='horizontal', size=(12, 12), key='-UpperThreshold-')]
        ]
