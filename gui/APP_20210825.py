...
from DobotFunction.Communication import (
    Connect_Disconnect,
    Operation,
    _OneAction,
    SetPoseAct,
...
from ImageProcessing.Binarization import (
    GlobalThreshold,
    AdaptiveThreshold,
    TwoThreshold,
)
    ...
    def Layout(self):
        ...
        Task = [
            [
                ...
                sg.InputCombo(
                    ("Task_1",), default_value="Task_1", size=(9, 1), key="-TaskNum-"
                ),
            ],
        ]
        ...
        # -------------------------------------------------------------- #
            ...
            if values["-TaskNum-"] == "Task_1":
                if (self.connection == True) and (type(self.cam) == cv2.VideoCapture):
                    if (
                        (not self.Alignment_1)
                        or (not self.Alignment_2)
